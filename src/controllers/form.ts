...
router.post(
  ...
  async (ctx: Context) => {
    try {
      ...
      const [items, total] =  await formKit.listForm(token, isStar, offset, limit)
...
router.post(
  ...
  async (ctx: Context) => {
    ...
    await formKit.setFormStatus(token, id, EFormStatus.ing)
    ctx.body = generateOk()
  },
)
...
router.post(
  ...
  async (ctx: Context) => {
    const token = ctx.cookies.get('token')
    const { id } = ctx.request.body
    await formKit.setFormStatus(token, id, EFormStatus.end)
    ctx.body = generateOk()
  },
)
...
router.post('/get', async (ctx: Context) => {
  try {
    const { id } = ctx.request.body
    const item = await formKit.getForm(id)
...
// 填写表单,done
router.post('/input', checkBody(checkFormInputReq), (ctx: Context) => {
  try {
    const { formId, problems } = ctx.request.body as IInputReq
    formResultKit.create(formId, problems)
    ctx.body = generateOk()
  } catch (err) {
    catchError(err, ctx)
  }
...
router.get(
  ...
  async (ctx: Context) => {
    try {
      ...
      const info = await formKit.getForm(formId)
      const items = await formResultKit.getFormResult(token, formId)
      ctx.body = generateOk({
        info,
        items,
      })
...
router.post(
  ...
  async (ctx: Context) => {
    try {
      const token = ctx.cookies.get('token')
      const { id } = ctx.params
      const item = await formResultKit.getDetail(token, id)
      ctx.body = generateOk({
        item,
      })
    } catch (err) {
